  ...
  describe("Adapter deployment parameters", function() {
    ...
    describe("Reverts with creating new orders", function () {
      ...
      it("Should revert if operation is invalid", async() => {
        ...
        .to.be.revertedWithCustomError(gmxAdapterLibrary.attach(traderWalletContract.address), "InvalidOperationId")
      });
    });
        ...
        describe("Creating Partial decrease limit order", function () {
          ...
          before(async () => {
            ...
            const tradeData = abiCoder.encode(
              ...
              [indexToken, sizeDelta, collateralToken, collateralDelta, isLong, triggerPrice, triggerAboveThreshold]
              );
          ...
          it("Should return correct data of created decrease limit order", async () => {
            ...
            const order = await gmxOrderBook.decreaseOrders(traderWalletContract.address, index);
            expect(order.account).to.equal(traderWalletContract.address);
            expect(order.collateralToken).to.equal(collateralToken);
          ...
          describe("Update existing decrease limit order", function () {
            ...
            before(async () => {
              ...
              const tradeData = abiCoder.encode(
                ...
                [[walletIndex, vaultIndex], collateralDelta, newSizeDelta, triggerPrice, newTriggerAboveThreshold]
              );
              const tradeOperation = { operationId, data: tradeData };
              txResult = await traderWalletContract.connect(trader).executeOnProtocol(
                protocolId,
          ...
          describe("Fail execution decrease limit order because price didn't reach trigger", function () {
            it("Should revert order execution because price didn't reach trigger", async () => {
              await expect(
                gmxPositionManager.connect(limitOrderKeeper)
                  .executeDecreaseOrder(traderWalletContract.address, 0, gmx.limitOrderKeeper)
                ).to.be.revertedWith("OrderBook: invalid price for execution");
            });
          });
          ...
          describe("Executing Long Decrease order by limitOrderKeeper", function () {
            before(async () => {
              ...
              await gmxVaultPriceFeedMock.setPrice(indexToken, triggerPrice.add(1));
              // execute order
              txResult = await gmxPositionManager.connect(limitOrderKeeper)
                .executeDecreaseOrder(traderWalletContract.address, 0, gmx.limitOrderKeeper);
            ...
            it("Should execute created decrease order", async () => {
              ...
              expect(collateralUsdValue)
                .to.be.gt(amountIn.sub(collateralDelta).sub(utils.parseUnits("100", 30)));  // position collateral in USD
              expect(collateralUsdValue).to.be.lt(amountIn.sub(collateralDelta));
            });
          });
        });
        ...
        describe("Creating Full decrease (close) limit order", function () {
          ...
          describe("Executing Long Decrease order by limitOrderKeeper", function () {
            ...
            it("Should return collateral tokens balance to contract", async () => {
              ...
              const collateralReturn = await wbtcTokenContract.balanceOf(traderWalletContract.address);
              const returnedBalanceUsd = collateralReturn.mul(collateralPrice).div(utils.parseUnits("1", 30));
